...
const handleStep1 = async () => {
  ...
  loading.value = true
  try {
    const auth_api = (await import('@/api_factory/modules/auth')).auth_api;
    await auth_api.updateProfile({ phone: form.phone, referredBy: form.referredBy })
    currentStep.value = 'business'
  } catch (e: any) { 
    error.value = e?.data?.message || e?.response?.data?.message || 'Update failed.' 
